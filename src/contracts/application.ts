...
export interface QuoteRevision { revision: string; lines: Line[]; totals: Total[]; orderDiscountPct: number; promisedDate: string | null; evaluation: Evaluation; at: string }
export interface Quote extends QuoteRevision { id: string; customerId: string; repId: string; name: string; currency: string; stage: string; sent: boolean; history: QuoteRevision[]; events: Event[]; requestedDate: string | null; dateReviewPending: boolean; orderId?: string; acceptedAt?: string }
