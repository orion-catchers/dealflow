...
export function Fields({fields,value,set,d}:{fields:Field[];value:Record<string,unknown>;set:(v:Record<string,unknown>)=>void;d?:DataState}){return <div className="form-grid">{fields.map(f=>{const update=(v:unknown)=>set({...value,[f.key]:v});if(f.type==='checkbox')return <label className="check" key={f.key}><input type="checkbox" checked={value[f.key]===true} onChange={e=>update(e.target.checked)}/>{f.label}</label>;const opts=selectChoices(f,d);if(f.type==='select')return <Select key={f.key} name={f.key} label={f.label} required={f.required} value={String(value[f.key]??'')} onChange={e=>update(e.target.value)}><option value="">Select…</option>{opts.map(o=><option value={o.id} key={o.id}>{o.name}</option>)}</Select>;if(f.type==='textarea')return <label key={f.key}>{f.label}<textarea value={String(value[f.key]??'')} required={f.required} onChange={e=>update(e.target.value)}/></label>;return <Input key={f.key} label={f.label} type={f.type??'text'} step={f.type==='number'?'any':undefined} required={f.required} min={f.min} max={f.max} value={String(value[f.key]??'')} onChange={e=>update(f.type==='number'&&e.target.value!==''?Number(e.target.value):e.target.value)}/>;})}</div>;}
