...
export interface Context {d:DataState;actor:Actor;path:string;reload:()=>Promise<void>;run:(action:string,body?:Record<string,unknown>,notice?:string)=>Promise<unknown>;notice:(s:string)=>void;}
export function Table({head,rows,empty='No matching records. Adjust your filters or create a record to get started.'}:{head:string[];rows:ReactNode[][];empty?:ReactNode}){return <div className="table-wrap" tabIndex={0} role="region" aria-label={head.join(', ')}><table><thead><tr>{head.map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{rows.length?rows.map((r,i)=><tr key={i}>{r.map((c,j)=><td key={j}>{c}</td>)}</tr>):<tr><td colSpan={head.length}><div className="empty">{empty}</div></td></tr>}</tbody></table></div>;}
...
export function Fields({fields,value,set,d}:{fields:Field[];value:Record<string,unknown>;set:(v:Record<string,unknown>)=>void;d?:DataState}){return <div className="form-grid">{fields.map(f=>{const update=(v:unknown)=>set({...value,[f.key]:v});if(f.type==='checkbox')return <label className="check" key={f.key}><input type="checkbox" checked={value[f.key]===true} onChange={e=>update(e.target.checked)}/>{f.label}</label>;const opts=selectChoices(f,d);if(f.type==='select')return <Select key={f.key} name={f.key} label={f.label} required={f.required} value={String(value[f.key]??'')} onChange={e=>update(e.target.value)}><option value="">Select…</option>{opts.map(o=><option value={o.id} key={o.id}>{o.name}</option>)}</Select>;if(f.type==='textarea')return <label key={f.key}>{f.label}<textarea value={String(value[f.key]??'')} required={f.required} onChange={e=>update(e.target.value)}/></label>;const moneyKey=f.key==='amount'||f.key==='unitPrice'||f.key==='listPrice';
return <Input key={f.key} label={f.label} type={moneyKey?'text':(f.type??'text')} inputMode={moneyKey||f.type==='number'?'decimal':undefined} step={f.type==='number'&&!moneyKey?(f.key==='quantity'?1:'any'):undefined} required={f.required} min={moneyKey?undefined:f.min} max={moneyKey?undefined:f.max} value={String(value[f.key]??'')} onChange={e=>update(f.type==='number'&&!moneyKey&&e.target.value!==''?(f.key==='quantity'?Math.max(f.min??1,Math.round(Number(e.target.value)||1)):Number(e.target.value)):e.target.value)}/>;})}</div>;}
export function FormAction({title,fields=[],initial={},onSubmit,d,button=title,danger=false,variant,description,confirmLabel}:{title:string;fields?:Field[];initial?:Record<string,unknown>;onSubmit:(v:Record<string,unknown>)=>Promise<unknown>;d?:DataState;button?:string;danger?:boolean;variant?:'primary'|'secondary'|'danger';description?:string;confirmLabel?:string}){const [open,setOpen]=useState(false),[value,set]=useState(initial),[busy,setBusy]=useState(false),[error,setError]=useState(''),[key,setKey]=useState('');const tone=variant??(danger?'danger':'secondary');return <><Button variant={tone} onClick={()=>{set(initial);setError('');setKey(newId());setOpen(true);}}>{button}</Button><Dialog open={open} onClose={()=>!busy&&setOpen(false)} title={title} footer={null}><form onSubmit={async e=>{e.preventDefault();setBusy(true);setError('');const posted={...value};const data=new FormData(e.currentTarget);for(const field of fields){if(field.type!=='select')continue;const next=data.get(field.key);if(typeof next==='string'&&next.trim()!=='')posted[field.key]=next;}try{await onSubmit({...posted,requestKey:key});setOpen(false);}catch(err){setError((err as Error).message);}finally{setBusy(false);}}}>{description&&<p>{description}</p>}<Fields fields={fields} value={value} set={v=>{set(v);setKey(newId());}} d={d}/>{error&&<p role="alert" className="error">{error}</p>}<div className="actions"><Button type="submit" disabled={busy} variant={tone==='danger'?'danger':'primary'}>{busy?'Saving…':confirmLabel??(fields.length?title:'Confirm')}</Button><Button type="button" variant="secondary" disabled={busy} onClick={()=>setOpen(false)}>Cancel</Button></div></form></Dialog></>;}
