...
describe.skipIf(!dbAvailable)("auth session flow", () => {
  ...
  it("login returns session user with fixture actor id", async () => {
    const { user, token } = await loginWithPassword({ email: "dev@nexa.example", password: "password123" });
  ...
  it("getActor resolves session cookie for active user", async () => {
    const { token } = await loginWithPassword({ email: "arjun@nexa.example", password: "password123" });
  ...
  it("getSessionUser returns customer fixture id", async () => {
    const { token } = await loginWithPassword({ email: "neha@acme.example", password: "password123" });
    const request = new Request("http://localhost/api/test", {
      headers: { cookie: `${SESSION_COOKIE}=${token}` },
    });
  ...
  it("pending user cannot login", async () => {
    const err = await failure(loginWithPassword({ email: "vikram@nexa.example", password: "password123" }));
    expect(err.code).toBe("UNAUTHENTICATED");
  });
