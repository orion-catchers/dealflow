...
function impersonationEnabled(): boolean {
  return process.env.DEALFLOW_DEV_IMPERSONATION === "1" && process.env.NODE_ENV !== "production";
}
...
async function impersonatedActor(request: Request): Promise<Actor | null> {
...
export async function getActor(request: Request): Promise<Actor> {
  ...
  if (impersonationEnabled()) {
    const impersonated = await impersonatedActor(request);
...
export async function getSessionUser(request: Request): Promise<SessionUser> {
  ...
  if (impersonationEnabled()) {
    const actor = await impersonatedActor(request);
    if (actor) return devFixtureToSessionUser(actor);
  }
