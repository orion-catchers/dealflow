...
export class LiveQuoteService {
  ...
  async revise(actor: Actor, quoteId: string, input: QuoteRevisionInput): Promise<Quote> {
    requireRole(actor, "ADMIN", "SALES_REP", "SALES_MANAGER");
    const actorId = await this.resolveActorUserId(actor, this.db);
    return this.db.$transaction(async (tx) => {
  ...
  async mutateLines(actor: Actor, quoteId: string, input: QuoteLineMutationInput): Promise<Quote> {
  ...
  async decideApproval(actor: Actor, input: ApprovalMutationInput): Promise<Quote> {
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (input.decision === "APPROVE") {
        ...
        if (!remaining) {
          await tx.quoteRevision.update({ where: { id: revision.id }, data: { approvalStatus: "APPROVED" } });
          await tx.quote.update({ where: { id: revision.quote.id }, data: { stage: "APPROVED", lastActivityAt: new Date() } });
        }
      } else if (input.decision === "REJECT") {
        await tx.quoteRevision.update({ where: { id: revision.id }, data: { approvalStatus: "REJECTED" } });
  ...
  async dashboard(actor: Actor) {
    ...
    const [pendingApprovals, openQuotes, atRiskDeals, recentEvents] = await Promise.all([
      this.db.quoteRevision.count({ where: { approvalStatus: "PENDING", quote: where } }),
  ...
  private async writeRevision(
    ...
    const resolvedLines: Array<QuoteLineInput & { categoryId: string; planId: string | null }> = [];
    for (const [index, draft] of input.lines.entries()) {
      ...
      resolvedLines.push({
        ...
        billingInterval: catalog.billingInterval as QuoteLineInput["billingInterval"],
  ...
  private async resolveCatalogLine(customer: { id: string; priceListId: string; currency: string }, draft: QuoteLineDraft, db: Client) {
