...
import {
  getLivePolicyService,
  snapshotForTier,
} from "@/server/governance/live-policy-service";
...
export interface QuoteLineMutationInput {
  expectedRevision: ExpectedRevision;
...
type RevisionInclude = {
  policyVersion: {
    id: string;
    createdAt: Date;
    tierCeilings: Array<{ tier: DiscountTier; ceilingPct: Prisma.Decimal }>;
...
const quoteInclude = {
  customer: {
    select: { id: true, name: true, discountTier: true, currency: true },
  },
...
function decimal(
  value: Prisma.Decimal | { toString(): string } | number,
): string {
...
function dateValue(value: string | null | undefined): Date | null {
  ...
  if (parsed.toISOString().slice(0, 10) !== value) {
    throw new ApiFailure(
      "INVALID_INPUT",
      "promisedDate is not a valid calendar date.",
    );
  }
  return parsed;
}
...
function policySnapshotFromStoredRevision(row: RevisionRow) {
  ...
  return stored && typeof stored === "object"
    ? (stored as QuoteRevision["evaluation"]["policySnapshot"])
    : snapshotForTier(row.policyVersion as never, "GOLD");
}

function toPricedLine(line: RevisionRow["lines"][number]): PricedQuoteLine {
  ...
  return {
    ...
    marginAmount: money(
      line.lineSubtotal.minus(
        new Prisma.Decimal(line.unitCost).times(line.quantity),
      ),
    ),
    marginPct: line.lineSubtotal.isZero()
      ? "0.00"
      : line.lineSubtotal
          .minus(new Prisma.Decimal(line.unitCost).times(line.quantity))
          .div(line.lineSubtotal)
          .times(100)
          .toFixed(2),
  };
}

function toRevision(row: RevisionRow): QuoteRevision {
  const lines = row.lines.map(toPricedLine);
  const raw = jsonObject(row.evaluationReasons);
  const storedEvaluation =
    raw.evaluation && typeof raw.evaluation === "object"
      ? (raw.evaluation as QuoteRevision["evaluation"])
      : {
          status:
            row.approvalStatus === "NOT_REQUIRED" ? "NOT_REQUIRED" : "PENDING",
          riskLevel: row.riskLevel,
          requiredApprovalChain: row.approvalSteps.map((step) =>
            step.role === "SALES_MANAGER" ? "MANAGER" : "FINANCE",
          ),
  ...
  const recurringTotals: Partial<
    Record<"MONTHLY" | "QUARTERLY" | "YEARLY", string>
  > = {};
  if (!new Prisma.Decimal(row.recurringMonthly).isZero())
    recurringTotals.MONTHLY = money(row.recurringMonthly);
  if (!new Prisma.Decimal(row.recurringQuarterly).isZero())
    recurringTotals.QUARTERLY = money(row.recurringQuarterly);
  if (!new Prisma.Decimal(row.recurringYearly).isZero())
    recurringTotals.YEARLY = money(row.recurringYearly);
  return {
    id: row.id,
    quoteId: row.quoteId,
    ...
    pricing: {
      ...
      totals: {
        ...
        marginTotal: lines
          .reduce(
            (sum, line) => sum.plus(line.marginAmount),
            new Prisma.Decimal(0),
          )
          .toFixed(2),
      ...
      effectiveDiscountPct: lines.length
        ? row.lines
            .reduce(
              (sum, line) =>
                sum.plus(
                  new Prisma.Decimal(line.effectiveDiscountPct).times(
                    new Prisma.Decimal(line.unitPrice).times(line.quantity),
                  ),
                ),
              new Prisma.Decimal(0),
            )
            .div(
              lines.reduce(
                (sum, line) => sum.plus(line.undiscountedAmount),
                new Prisma.Decimal(0),
              ),
            )
            .toFixed(2)
    ...
    evaluation: storedEvaluation as QuoteRevision["evaluation"],
    approvalStatus: row.approvalStatus,
    customerAcceptedAt: row.acceptances[0]
      ? row.acceptances[0].createdAt.toISOString()
      : undefined,
...
function toQuote(row: QuoteRow): Quote {
  ...
  if (!current)
    throw new ApiFailure(
      "CONFLICT",
      `Quote ${row.id} has no current revision.`,
    );
  return {
    id: row.id,
...
function quoteStageForEvaluation(
  status: "NOT_REQUIRED" | "PENDING",
  draftStage: QuoteStage = "DRAFT",
): QuoteStage {
  if (draftStage === "UNDER_NEGOTIATION")
    return status === "PENDING" ? "PENDING_APPROVAL" : "UNDER_NEGOTIATION";
  return draftStage;
}
...
export class LiveQuoteService {
  ...
  async create(actor: Actor, input: QuoteCreateInput): Promise<Quote> {
    ...
    if (actor.role === "SALES_REP" && customer.assignedRepId !== actorId) {
      throw new ApiFailure(
        "FORBIDDEN",
        "Sales reps may only quote their assigned customers.",
      );
    }
  ...
  async revise(
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (quote.stage === "CONFIRMED")
        throw new ApiFailure("CONFLICT", "Confirmed quotes cannot be edited.");
      ...
      await this.writeRevision(tx, quote, actorId, {
        ...
        lines,
        orderDiscountPct:
          input.orderDiscountPct ?? decimal(current.orderDiscountPct),
        promisedDate:
          input.promisedDate === undefined
            ? dateOnly(current.promisedDate)
            : input.promisedDate,
        stage:
          quote.stage === "UNDER_NEGOTIATION" ? "UNDER_NEGOTIATION" : "DRAFT",
      });
      return toQuote(
        (await tx.quote.findUniqueOrThrow({
          where: { id: quote.id },
  ...
  async mutateLines(
    ...
    const current = quote.revisions.find(
      (revision) => revision.id === quote.currentRevisionId,
    )!;
    let lines: QuoteLineDraft[] = current.lines.map((line) => ({
      productId: line.productId,
    ...
    if (input.action === "ADD") {
      if (!input.line)
        throw new ApiFailure("INVALID_INPUT", "line is required for ADD.");
      lines.push(input.line);
    } else if (input.action === "REMOVE") {
      if (!input.lineId)
        throw new ApiFailure("INVALID_INPUT", "lineId is required for REMOVE.");
      const before = lines.length;
      lines = lines.filter(
        (_, index) => current.lines[index]?.lineId !== input.lineId,
      );
      if (lines.length === before)
        throw new ApiFailure("NOT_FOUND", "Quote line not found.");
    } else {
      if (!input.lines)
        throw new ApiFailure("INVALID_INPUT", "lines is required for REPLACE.");
      lines = input.lines;
    }
    if (!lines.length)
      throw new ApiFailure(
  ...
  async submit(
    ...
    return this.db.$transaction(async (tx) => {
      ...
      const stage =
        quote.currentRevision!.approvalStatus === "NOT_REQUIRED"
          ? "APPROVED"
          : "PENDING_APPROVAL";
      await tx.quote.update({
        where: { id: quote.id },
  ...
  async send(
    actor: Actor,
    quoteId: string,
    input: SendQuoteInput | ExpectedRevision,
  ): Promise<Quote> {
    requireRole(actor, "ADMIN", "SALES_REP", "SALES_MANAGER");
    const actorId = await this.resolveActorUserId(actor, this.db);
    return this.db.$transaction(async (tx) => {
      const quote = await this.lockVisibleQuote(actor, quoteId, tx);
      const sendInput =
        typeof input === "object" ? input : { expectedRevision: input };
      this.assertExpectedRevision(quote, sendInput.expectedRevision);
      if (quote.stage !== "APPROVED")
        throw new ApiFailure("CONFLICT", "Only approved quotes can be sent.");
      if (
        sendInput.customerTier &&
        actor.role === "SALES_REP" &&
        quote.repId !== actorId
      ) {
        throw new ApiFailure(
          "FORBIDDEN",
          "Sales reps may only assign tiers for their own quotes.",
        );
      }
      await tx.quote.update({
        where: { id: quote.id },
        data: {
          ...
          ...(sendInput.customerTier
            ? {
                customer: {
                  update: {
                    // The public contract calls the entry Bronze; the existing
                    // Prisma enum stores that tier as STANDARD.
                    discountTier: tierToDb(sendInput.customerTier),
                  },
                },
              }
            : {}),
      ...
      await recordPrismaAudit(tx, {
        ...
        actorId,
        action: "QUOTE_SENT",
        metadata: sendInput.customerTier
          ? { customerTier: sendInput.customerTier }
          : {},
      });
      return toQuote(
        (await tx.quote.findUniqueOrThrow({
  ...
  async accept(
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (
        ...
        revision.approvalStatus === "SUPERSEDED"
      ) {
        throw new ApiFailure(
          "CONFLICT",
      ...
      const existing = revision.acceptances.find(
  ...
  async propose(
    ...
    if (!input.requestKey.trim())
      throw new ApiFailure("INVALID_INPUT", "requestKey is required.");
    if (
      !input.body.trim() &&
      !input.lineChanges.some(
        (line) =>
          line.comment?.trim() ||
      ...
      throw new ApiFailure(
        "INVALID_INPUT",
        "Proposal body or a requested change is required.",
      );
    }
    const actorId = await this.resolveActorUserId(actor, this.db);
    return this.db.$transaction(async (tx) => {
      ...
      if (prior) {
        ...
        if (meta.fingerprint !== fingerprint || meta.quoteId !== quoteId) {
          throw new ApiFailure(
            "CONFLICT",
            "Request key belongs to another proposal.",
          );
        }
        return {
      ...
      const numeric = input.lineChanges.filter(
        (line) => line.quantity !== undefined || line.discountPct !== undefined,
      );
      ...
      if (numeric.length || input.requestedDeliveryDate) {
        const created = await this.writeRevision(tx, quote, actorId, {
          ...
          promisedDate:
            input.requestedDeliveryDate ?? dateOnly(current.promisedDate),
          stage: "UNDER_NEGOTIATION",
        });
        revision = created;
      }
      const message = await tx.portalMessage.create({
        data: {
          ...
          status: "OPEN",
          proposedPromisedDate: input.requestedDeliveryDate
            ? dateValue(input.requestedDeliveryDate)
            : null,
  ...
  async decideApproval(
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (revision.quote.currentRevisionId !== revision.id)
        throw new ApiFailure(
          "CONFLICT",
      ...
      if (revision.approvalStatus !== "PENDING")
        throw new ApiFailure(
          "CONFLICT",
          "This revision is not awaiting approval.",
        );
      if (revision.createdById === actorId)
        throw new ApiFailure(
          "FORBIDDEN",
          "You cannot approve a revision you authored.",
        );
      if (input.requestKey) {
        const prior = await tx.auditEvent.findFirst({
          where: {
        ...
        if (prior)
          return toQuote(
            (await tx.quote.findUniqueOrThrow({
              where: { id: revision.quote.id },
              include: quoteInclude,
            })) as unknown as QuoteRow,
          );
      }
      const pending = [...revision.approvalSteps]
        .sort((a, b) => a.stepIndex - b.stepIndex)
        .find((step) => step.status === "PENDING");
      if (!pending)
        throw new ApiFailure("CONFLICT", "No pending approval step.");
      if (actor.role !== "ADMIN" && actor.role !== pending.role) {
        throw new ApiFailure(
          "FORBIDDEN",
          `Role ${actor.role} may not act on this step.`,
        );
      }
      ...
      if (input.decision === "APPROVE") {
        ...
        const remaining = revision.approvalSteps.some(
          (step) =>
            step.stepIndex !== pending.stepIndex && step.status === "PENDING",
        );
        if (!remaining) {
          ...
          await tx.quote.update({
            where: { id: revision.quote.id },
            data: { stage: "APPROVED", lastActivityAt: new Date() },
          });
        }
        ...
        await tx.quote.update({
          where: { id: revision.quote.id },
          data: { stage: "REJECTED", lastActivityAt: new Date() },
        });
        await tx.quoteRevisionApprovalStep.updateMany({
        ...
        await tx.quote.update({
          where: { id: revision.quote.id },
          data: { stage: "UNDER_NEGOTIATION", lastActivityAt: new Date() },
        });
      }
      await recordPrismaAudit(tx, {
        entityType: "QuoteRevision",
  ...
  async confirm(
    ...
    return this.db.$transaction(async (tx) => {
      ...
      if (request?.completedAt && request.resultPayload) {
        ...
        if (
          payload.sourceQuoteId !== quoteId ||
          payload.customerId !== actor.customerId
        ) {
          throw new ApiFailure(
            "CONFLICT",
            "Request key belongs to another confirmation.",
          );
        }
      ...
      if (request)
        throw new ApiFailure(
          "CONFLICT",
          "Confirmation request is already in progress.",
        );
      const quote = await this.lockVisibleQuote(actor, quoteId, tx);
      await this.assertCustomerMembership(actor, quote.customerId, tx);
      this.assertExpectedRevision(quote, input.expectedRevision);
      ...
      if (
        !revision.acceptances.some(
          (acceptance) => acceptance.actorId === actorId,
        )
      ) {
        throw new ApiFailure(
          "CONFLICT",
      ...
      const order = await tx.order.create({
        data: {
          sourceRevisionId: revision.id,
          acceptanceId: revision.acceptances.find(
            (acceptance) => acceptance.actorId === actorId,
          )!.id,
          customerId: quote.customerId,
          repId: quote.repId,
      ...
      await tx.quote.update({
        where: { id: quote.id },
        data: { stage: "CONFIRMED", lastActivityAt: new Date() },
      });
      ...
      const fulfillmentInput: OrderForFulfillment = {
        ...
        lines: order.lines.map((line) => ({
          ...
          shippingWeightKg: line.variant?.shippingWeight
            ? Number(line.variant.shippingWeight.toString())
            : undefined,
        })),
      };
      ...
      const fulfillmentInitialization = await initializeFulfillment(
        tx,
        orderReady,
      );
      const payload: ConfirmOrderResult = {
  ...
  async dashboard(actor: Actor) {
    ...
    const where = await this.quoteScope(actor, this.db);
    const [pendingApprovals, openQuotes, atRiskDeals, recentEvents] =
      await Promise.all([
        this.db.quoteRevision.count({
          where: { approvalStatus: "PENDING", quote: where },
        }),
        this.db.quote.count({
          where: { ...where, stage: { notIn: ["CONFIRMED", "REJECTED"] } },
        }),
        this.db.healthFlag.count({
          where: {
            ...
            OR: [
              { quote: where },
              { order: { sourceRevision: { quote: where } } },
            ],
          },
        }),
        this.db.auditEvent.findMany({
          where: {
            actorId:
              actor.role === "SALES_REP"
                ? await this.resolveActorUserId(actor, this.db)
                : undefined,
          },
          orderBy: { createdAt: "desc" },
          take: 10,
  ...
  private async writeRevision(
    ...
    if (input.expectedRevision !== (current?.revisionNumber ?? 0)) {
      throw new ApiFailure("CONFLICT", "Quote revision is stale.", {
    ...
    if (!policyRow)
      throw new ApiFailure(
        "INVALID_INPUT",
        "No published policy version exists.",
      );
    const policySnapshot = snapshotForTier(policyRow, customer.discountTier);
    const resolvedLines: Array<
      QuoteLineInput & { categoryId: string; planId: string | null }
    > = [];
    for (const [index, draft] of input.lines.entries()) {
      ...
      resolvedLines.push({
        ...
        billingInterval:
          catalog.billingInterval as QuoteLineInput["billingInterval"],
    ...
    const revision = await tx.quoteRevision.create({
      data: {
        ...
        approvalStatus:
          evaluation.status === "PENDING" ? "PENDING" : "NOT_REQUIRED",
        orderDiscountPct: priced.orderDiscountPct,
        currency: priced.totals.currency,
        promisedDate: dateValue(input.promisedDate),
        oneTimeSubtotal: priced.lines
          .filter((line) => line.billingInterval === "ONE_TIME")
          .reduce(
            (sum, line) =>
              sum.plus(line.undiscountedAmount).minus(line.discountAmount),
            new Prisma.Decimal(0),
          ),
        oneTimeTax: priced.lines
          .filter((line) => line.billingInterval === "ONE_TIME")
          .reduce(
            (sum, line) => sum.plus(line.taxAmount),
            new Prisma.Decimal(0),
        ...
        lines: {
          create: priced.lines.map((line, position) => {
            const breach = evaluation.breaches.find(
              (item) => item.lineId === line.lineId,
            );
            const category = line.category
              ? policySnapshot.rules.categoryCeilingsPct[line.category]
              : undefined;
            const ceiling = category
              ? Math.min(
                  Number(policySnapshot.rules.defaultCeilingPct),
                  Number(category),
                )
              : Number(policySnapshot.rules.defaultCeilingPct);
            return {
              ...
              billingKind:
                line.billingInterval === "ONE_TIME" ? "ONE_TIME" : "RECURRING",
              interval:
                line.billingInterval === "ONE_TIME"
                  ? null
              ...
              taxPct: line.taxPct,
              lineSubtotal: new Prisma.Decimal(line.undiscountedAmount).minus(
                line.discountAmount,
              ),
              taxAmount: line.taxAmount,
    ...
    // QuoteLine.categoryId is a foreign key and needs the catalog category row.
  ...
  private async resolveCatalogLine(
    customer: { id: string; priceListId: string; currency: string },
    ...
    if (!product || product.archivedAt)
      throw new ApiFailure(
        "NOT_FOUND",
        `Product ${draft.productId} is not available.`,
      );
    const variant = draft.variantId
      ? await db.variant.findUnique({
          where: { id: await this.resolveVariantId(draft.variantId, db) },
        })
      : null;
    if (
      draft.variantId &&
      (!variant || variant.productId !== product.id || variant.archivedAt)
    ) {
      throw new ApiFailure(
        "NOT_FOUND",
        `Variant ${draft.variantId} is not available on product ${draft.productId}.`,
      );
    }
    const tier = (
    ...
    const specific = variant
      ...
      : null;
    const generic = await db.priceRule.findFirst({
      where: {
        priceListId: customer.priceListId,
        productId: product.id,
        variantId: null,
    ...
    const unitPrice = new Prisma.Decimal(
      specific?.unitPrice ?? generic?.unitPrice ?? product.basePrice,
    ).plus(variant?.extraPrice ?? 0);
    return {
      productId: product.id,
      variantId: variant?.id,
      ...
      billingInterval: product.defaultPlan
        ? product.defaultPlan.interval
        : "ONE_TIME",
  ...
  private async loadVisibleQuote(
    actor: Actor,
    quoteId: string,
    db: Client,
  ): Promise<QuoteRow> {
  ...
  private async lockVisibleQuote(
    actor: Actor,
    quoteId: string,
    db: Tx,
  ): Promise<QuoteRow> {
  ...
  private async quoteScope(
    ...
    if (actor.role === "SALES_MANAGER") {
      ...
      if (!user?.teamId)
        throw new ApiFailure(
          "FORBIDDEN",
          "Sales manager has no assigned team.",
        );
  ...
  private assertExpectedRevision(quote: QuoteRow, expected: ExpectedRevision) {
    ...
    if (
      ...
      (typeof expected === "number"
        ? current.revisionNumber !== expected
        : current.id !== expected &&
          String(current.revisionNumber) !== expected)
    ) {
      throw new ApiFailure(
        "CONFLICT",
        "Quote revision is stale. Reload and review the current revision.",
        {
  ...
  private async resolveActorUserId(actor: Actor, db: Client): Promise<string> {
    ...
    if (!row)
      throw new ApiFailure("UNAUTHENTICATED", "Actor is not a database user.");
    return row.id;
  }

  private async customerIdsForActor(
    actor: Actor,
    db: Client,
  ): Promise<string[]> {
    ...
    if (!memberships.length)
      throw new ApiFailure(
        "FORBIDDEN",
        "Customer has no active customer membership.",
      );
    return memberships.map((membership) => membership.customerId);
  }

  private async assertCustomerMembership(
    actor: Actor,
    customerId: string,
    db: Client,
  ) {
    ...
    if (!ids.includes(customerId))
      throw new ApiFailure("FORBIDDEN", "Customer cannot access this quote.");
  }
  ...
  private async resolveCustomer(id: string, db: Client) {
    ...
    const fixture = (
      await import("@/fixtures/harsh")
    ).harshFixtures.customers.find((customer) => customer.sym === id);
    if (fixture) {
      const mapped = await db.customer.findFirst({
        where: { contactEmail: fixture.contactEmail },
      });
  ...
  private async resolveProductId(id: string, db: Client): Promise<string> {
    ...
    const fixture = (
      await import("@/fixtures/harsh")
    ).harshFixtures.products.find((product) => product.sym === id);
    if (!fixture) return id;
    const mapped = await db.product.findUnique({
  ...
  private async resolveVariantId(id: string, db: Client): Promise<string> {
    if (await db.variant.findUnique({ where: { id }, select: { id: true } }))
      return id;
    const fixture = (
      await import("@/fixtures/harsh")
    ).harshFixtures.variants.find((variant) => variant.sym === id);
    if (!fixture) return id;
    const mapped = await db.variant.findUnique({
      where: { sku: fixture.sku },
      select: { id: true },
    });
    return mapped?.id ?? id;
