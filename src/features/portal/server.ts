...
import type { Actor, ApplicationAdapter, DataState, ProposalInput, Quote, DealRevision } from '../../contracts/application';
...
export function portalData(actor: Actor, state: DataState) {
  const customerId = customerActor(actor);
  const quotes = state.quotes.filter(q => q.customerId === customerId && q.sent);
  const ids = new Set(quotes.map(q => q.id));
  return {
    quotes: quotes.map(safeQuote),
    messages: state.messages.filter(m => ids.has(m.quoteId)).map(m => ({ id: m.id, quoteId: m.quoteId, revision: m.revision, lineId: m.lineId, senderName: m.senderName, text: m.text, at: m.at, kind: m.kind, requestedDate: m.requestedDate })),
    proposals: state.proposals.filter(p => ids.has(p.quoteId)).map(p => ({ id: p.id, quoteId: p.quoteId, fromRevision: p.fromRevision, proposedRevision: p.proposedRevision, at: p.at, status: p.status, requestedDate: p.requestedDate, lineChanges: p.lineChanges.map(l => ({ lineId: l.lineId, quantity: l.quantity, discountPct: l.discountPct, comment: l.comment })) })),
    orders: state.orders.filter(o => o.customerId === customerId).map(o => ({ id: o.id, quoteId: o.quoteId, revision: o.revision, status: o.status, promisedDate: o.promisedDate, lines: o.lines.map(l => ({ id: l.id, description: l.description, quantity: l.quantity })), backorders: o.backorders.map(b => ({ lineId: b.lineId, quantity: b.quantity })) })),
    invoices: state.invoices.filter(i => i.customerId === customerId).map(i => ({
      id: i.id, orderId: i.orderId, currency: i.currency, dueDate: i.dueDate, status: i.status, net: i.net, tax: i.tax, total: i.total, paid: i.paid, credited: i.credited, outstanding: i.outstanding,
      lines: i.lines.map(l => ({ id: l.id, description: l.description, quantity: l.quantity, unitPrice: l.unitPrice, discountPct: l.discountPct, net: l.net, tax: l.tax, total: l.total })),
      payments: state.payments.filter(p => p.invoiceId === i.id).map(p => ({ id: p.id, amount: p.amount, method: p.method, reference: p.reference, date: p.date, recordedByName: p.recordedByName })),
    })),
  };
}
